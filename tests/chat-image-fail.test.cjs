...
test('server: image-turn config retried on any upstream failure, failures flushed to the owner log, keepalive on image turns', () => {
  ...
  // v-err-acct: والرسالة تسمّي الحساب الذي رفض والموديل (أيّ رصيد نفد)
  assert.ok(chat.includes("await logErrorAndFlush('chat/upstream-fail', new Error(upstream.status + ' ' + __acct() + ': ' + errText), { action: lastUserHasImage ? 'image-turn' : 'text-turn' });"), 'الفشل النهائيّ يُسجَّل بنوع الدور');

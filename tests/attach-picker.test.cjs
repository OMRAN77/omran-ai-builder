...
// يظهر شريحةً بدل الصمت.
{
  ...
  // v-plan-routing رفعه إلى 676 بعد هذا الإصلاح؛ كلّ ما يلزم هنا أنّه ≥ 675.
  assert.ok(Number((fs.readFileSync(path.join(root, 'js', 'app-04-i18n-state.js'), 'utf8').match(/'\.js\?v=(\d+)'/) || [])[1]) >= 675, 'وسم ملفّات اللغات رُفع');

...
test('٢. parseEspn + windowMatches: مباراة صالحة فقط، ونافذة ٣ ساعات مضت إلى ٣٦ قادمة بلا تكرار', async () => {
  ...
  assert.deepEqual(got.map((m) => m.id), ['1', '4', '5'], 'تاريخ تالف أو خصم ناقص = يُسقط');
  ...
  assert.deepEqual(w.map((m) => m.id), ['4', '1'], 'جارية قبل ساعتين تبقى، وبعد ٣٦ ساعة تسقط، والتكرار يسقط');
...
test('٥. مفاتيح الجدول في الـ14 لغة ووسم اللغات رُفع', () => {
  const K = ['tvMatchesToday', 'tvMatchLive', 'tvMatchPaid', 'tvNoMatches', 'tvMatchesNote'];

...
test('٦. الإرشاد بين المواقع: خطوات بأسماء الأزرار، وتحت كلّ خطوة رابط صفحتها من البحث', () => {
  const c = read('api/_lib/chat.js');
  // v-site-guide2: رابط الصفحة نفسها تحت كلّ خطوة، والنقل لموقع آخر برابطه، ومن نتائج البحث فقط
  assert.ok(c.includes('[الإرشاد بين المواقع والصفحات'));
  assert.ok(c.includes('وتحتها مباشرةً سطر مستقلّ فيه رابط تلك الصفحة نفسها'));
  assert.ok(c.includes('«هذي الخدمة ما هي هنا، ادخل هنا ↓»'));
  assert.ok(c.includes('الروابط من نتائج البحث أو الصفحات التي فتحتها فقط'));
  assert.ok(!c.includes('ممنوع رابط عميق لم تفتحه'), 'القاعدة القديمة (رابط البداية وحده) أُزيلت');
});

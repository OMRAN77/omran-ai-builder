...
test('workflow: dispatch inputs, Claude Code action with the repo key, ci before push, report line, guards', () => {
  ...
  for (const frag of ['workflow_dispatch:', 'task:', 'branch:', 'issue:', 'base:', 'anthropics/claude-code-action@v1', 'RAW_KEY: ${{ secrets.ANTHROPIC_API_KEY }}', "anthropic_api_key: ${{ env.KEY_KIND == 'api' && env.ANTHROPIC_KEY_CLEAN || '' }}", '${{ inputs.task }}', '${{ inputs.branch }}', 'npm run ci', 'git push -u origin "$TASK_BRANCH"', '<!-- omran-agent {"phase":"start"', '<!-- omran-agent {"phase":"done"', 'concurrency:', 'timeout-minutes:', 'if: always()', 'gh issue comment "$TASK_ISSUE"']) {

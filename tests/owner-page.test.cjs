...
test('٣. العنوان بالـ١٤ لغة، والقائمة تُعاد بناؤها مع الدخول والخروج، ووسوم الكاش ارتفعت', () => {
  ...
  assert.match(html, /js\/partials-settings\.js\?v=675/);
  assert.match(html, /css\/tokens\.css\?v=724/);
  assert.match(read('js/app-04-i18n-state.js'), /\.js\?v=697'/); // v-reply-export رفع الوسم (قبله 686)
});

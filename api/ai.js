...
function injectNote(action, body, country) {
  ...
  const isGuide = withImages && GUIDE_RE.test(userText);
  ...
  if (mode === 'balanced') {
    ...
    if (withImages && !isGuide) n += IMAGE_ANALYSIS_NOTE;
  ...
  if (!isPremium) {
    ...
    if (withImages) {
      if (!isGuide) note += IMAGE_ANALYSIS_NOTE; // v-guide: انظر التعليق في فرع balanced

...
function load(action) {
  switch (action) {
    ...
    case 'models': return require('./_lib/provider-models.js'); // v-provider-models: قائمة الموديلات الحيّة للمالك
...
// v-guide — نسخة serverNote بلا «قاعدة الموضوع» — تُستخدم عند وجود لقطة شاشة،
// لأن تلك القاعدة هي سبب ضياع الجهاز واللغة والهدف بين أدوار الإرشاد.
function serverNoteForGuide(country) {
  return serverNote(country).replace('\n[قاعدة الموضوع]: أجب على آخر رسالة فقط. إذا غيّر الموضوع اتبعه فورًا.', '');
}
...
const IMAGE_ANALYSIS_NOTE = '\n\n[قاعدة تحليل الصور — إلزامية مطلقة]: المستخدم أرفق صورة. ممنوع منعًا باتًا رفض تحليلها أو الرد بعبارات مثل "آسف لا أستطيع" أو "لا يمكنني تحليل هذه الصورة". حلّلها وصفيًا وتعليميًا بأفضل ما تستطيع. إذا كانت صورة طبية (جلد، جرح، أشعة، تقرير مختبر، دواء): صف ما يظهر فيها بدقة، اشرح الاحتمالات الشائعة لما يظهر بأسلوب تثقيفي، أعطِ نصيحة عملية للعناية، واختم بسطر واحد فقط أن التقييم النهائي يحتاج طبيبًا. هذا وصف تعليمي وليس تشخيصًا نهائيًا، وتقديمه لا يخالف أي سياسة. ولا يعني هذا أن تخترع ما لا تراه: إذا لم تستطع قراءة تفصيل في الصورة فقل ذلك صراحةً — الاعتراف بأن تفصيلًا غير واضح ليس رفضًا، والتخمين المقدَّم كأنه قراءة هو الخطأ الحقيقي.';

// v-guide — 🧭 إرشاد من لقطة الشاشة.
// سبب وجودها: الردود كانت تفقد الجهاز واللغة والهدف بين الأدوار، فتعطي
// اختصارات كيبورد لمستخدم على الجوال، وأسماء أزرار إنجليزية لواجهة عربية.
const GUIDE_RE = /(وش اسوي|شو اسوي|وش أسوي|شو الي اسوي|الخطوة|خطوة|كيف اسوي|كيف أسوي|كيف اعدل|وين اضغط|وين أضغط|ارشدني|أرشدني|ما فهمت|مافهمت|next step|where do i click|what do i do)/i;

const SCREEN_GUIDE_NOTE = '\n\n[إرشاد من لقطة الشاشة — إلزامي]:' +
  '\n• اعتمد على ما يظهر في اللقطة فقط، لا على ذاكرتك عن شكل البرنامج في أجهزة أخرى.' +
  '\n• احفظ سياق المحادثة كاملًا: نوع الجهاز، لغة الواجهة، اسم البرنامج، هدف المستخدم، وكل خطوة سبق أن أعطيتها — حتى لو لم يذكرها في رسالته الأخيرة. الرسالة القصيرة مثل "ما فهمت" تعني إعادة الشرح لنفس الموقف، لا بدء موضوع جديد.' +
  ...
  '\n• إذا لم تجد العنصر في الصورة فقل ذلك واطلب لقطة أوضح — ممنوع "دوّر على" أو "جرّب تضغط" أو "أحيانًا".' +
  '\n• ممنوع تخمين اسم البرنامج. اذكره فقط إذا كان مكتوبًا في اللقطة أو ذكره المستخدم؛ وإلا فقل «لم أتبيّن اسم البرنامج» واسأله عنه. ممنوع «يبدو أنه…» و«أو تطبيق مشابه».' +
  '\n• ممنوع وصف أي عنصر لا تراه فعلًا في اللقطة (أيقونة، سجل، مقبض، تبويب). إذا كان خارج الإطار فقل ذلك.' +
  ...
  '\n• الفشل الممنوع هنا هو إرسال المستخدم إلى زر خاطئ، لا الاعتراف بعدم وجود الميزة.';
 ...
 *
 * v-guide — withImages: عند وجود لقطة شاشة نُسقط «قاعدة الموضوع». تلك القاعدة
 * تأمر النموذج صراحةً بتجاهل ما قبل الرسالة الأخيرة، فيضيع الجهاز واللغة
 * والهدف بين أدوار الإرشاد — وهذا سبب ردود «اضغط Delete من الكيبورد» لمستخدم
 * قال قبل دورين إنه على الجوال.
 */
function balancedNote(action, country, withImages) {
  const topic = withImages ? '' : '\n[قاعدة الموضوع]: أجب على آخر رسالة فقط. إذا غيّر الموضوع اتبعه فورًا.';
  return noteDate() + noteCountry(country) + BIDI_RULE + topic;
}
...
function injectNote(action, body, country) {
  ...
  // v-guide — تُحسب مرة واحدة وتُستخدم في الوضعين.
  const withImages = hasAttachedImages(action, body);
  // v-guide: أي صورة مرفقة تُفعّل انضباط الإرشاد. تخمين النيّة من نصّ الرسالة
  // (GUIDE_RE) كان يُسقطه على رسائل مثل «كيف أغيّر المكان» فيعود النموذج
  // للتخمين: «يبدو Planner 5D» و«غالبًا فيه مقبض دوران».
  const isGuide = withImages;
  ...
  if (mode === 'balanced') {
    let n = balancedNote(action, country, withImages);
    // v-guide: كانت تُتجاهل تمامًا في balanced. وتُستثنى في الإرشاد لأن جملتها
    // «الرفض هو الفشل الوحيد الممنوع» تدفع النموذج لاختراع أداة بدل قول «غير موجودة».
    if (withImages) n += IMAGE_ANALYSIS_NOTE;
    if (isGuide) n += SCREEN_GUIDE_NOTE;
    applyNote(action, body, n + toneNote + intentNote + APP_FACTS_NOTE + HIVE);
    return;
  }
  ...
  // v-guide: مع اللقطات نستخدم النسخة بلا «قاعدة الموضوع».
  let note = withImages ? serverNoteForGuide(country) : serverNote(country);
  if (!isPremium) {
    if (action === 'claude') note += CLAUDE_STYLE_NOTE;
    else note += ORIGINAL_PERSONA_NOTE;
  }
  note += noteIdentity(action);
  // v-guide: الإرشاد من اللقطة يسري حتى في الوضع الاحترافي — الدقة هنا ليست أسلوبًا.
  if (isGuide) note += SCREEN_GUIDE_NOTE;
  if (!isPremium) {
  ...
  note += HIVE; // v545
  // v-guide: لا نلصق «وفصل السياق» داخل رسالة المستخدم عند وجود لقطة — النماذج
  // توزن هذا السطر أعلى من نص السيستم، فيمحو سياق الإرشاد كله.
  if (!withImages) tagLastUserMessage(action, body);

...
const TOOLS_NOTE = '\n\n[أدواتك الحقيقية — سبع، وهي تعمل فعلًا الآن]:\n' +
  ...
  '• [الصورة المرفقة — اقرأها أولًا، لا تعِد رسمها بلا طلب]: إذا كانت الصورة المرفقة لقطة شاشة أو مستندًا أو واجهة تطبيق أو رسمًا بيانيًا أو جدولًا أو فاتورة أو نموذجًا أو رسالة أو أي صورة معلوماتية، وكان المستخدم يسأل عنها أو يريد فهمها أو متابعة إجراء فيها أو مساعدة على استعمالها = اقرأ محتواها وأجبه نصًّا كمساعد بصري دقيق. ممنوع منعًا باتًا في هذه الحالة أن تعيد رسمها بـedit_image أو تحوّلها فيديو بـgenerate_video ما لم يطلب المستخدم صراحةً تغيير أسلوبها أو إعادة رسمها أو تحريكها. إعادة رسم لقطة شاشة/مستند أو تحويله فيديو دون طلب صريح = خطأ فادح يفسد ما يحتاجه المستخدم. اعرض التعديل أو التحريك كعرض اختياري بعد جوابك النصّي إن ناسب، لا أن تبادر به.\n' +
  '• web_search — أي سعر أو خبر أو طقس أو نتيجة أو رسوم رسمية أو معلومة قد تكون تغيّرت: ابحث أولًا.\n' +
  '• [الأجهزة والمنتجات — دقّة + فائدة قصوى (يشمل كل الأنواع والماركات لا نوعًا واحدًا)]: أي سؤال عن أي جهاز أو منتج (تلفزيون، مكيّف، ريموت، جوال، غسالة، ثلاجة، سيارة، أداة، إلكترونيات… أي ماركة) — سواء عن مواصفة أو سعر أو وظيفة زر أو تنظيف أو كود عطل أو توصيل = ابحث أولًا وأجب من مصدر حقيقي (دليل/موقع الشركة). ممنوع منعًا باتًا تخمين زرّ أو خطوة أو رقم بثقة («بالغالب زر HEALTH…»).\n' +
  '  وإذا لم يظهر رقم الموديل الدقيق (في الصورة أو النص): لا تكتفِ بـ«ما أقدر». كن مفيدًا لأقصى حد: (١) سمِّ الماركة والفئة بدقّة مما تراه، (٢) ابحث عن الماركة/الفئة وأعطِ السلسلة الشائعة في السوق ومدى المواصفات المتوقّع (الأحجام/الدقة/الميزات) مع توضيح أنها عامّة تقريبية لا مؤكّدة لهذا الجهاز بالذات، (٣) وضّح أن رقم الموديل هو ما يعطي المواصفات الدقيقة، ثم اطلب من المستخدم تصوير الملصق (خلف/أسفل الجهاز: Model No.) أو الفاتورة. القاعدة تنطبق على كل الأجهزة والماركات بلا استثناء. الصدق مع فائدة عامّة أفضل من رقم مخترع، وأفضل من «ما عندي معلومات».\n' +
  ...
  '\n[شكل ردّ الصورة — إلزاميّ (أمر المالك)]:\n' +
  '(ز١) عند توليد أو تعديل صورة: الصورة أوّلًا. ممنوع أي مقدّمة أو شرح أو سؤال قبلها. ممنوع منعًا باتًا أن تسأل «هل تفضّل تعديل الألوان أم إضافة نص؟» أو «هل تريد أن أبدأ؟» أو أي سؤال قبل إظهار الصورة — نفّذ فورًا بأفضل اجتهادك ثمّ اعرض النتيجة.\n' +
  '(ز٢) مع الصورة سطر واحد قصير فقط (مثل «تفضّل 👇»). ممنوع الفقرات الطويلة، وممنوع أي تحذير مثل «قد تكون الصور غير دقيقة/مضلّلة».\n' +
  '(ز٣) أي اقتراح إضافي يوضع بعد الصورة في سطر واحد مختصر كعرض اختياريّ لا سؤال إجباريّ (مثل: «أقدر أقوّيها أكثر أو أضيف نصًّا إذا تحب»). لا قائمة خيارات ولا استئذان.\n' +
  '(ز٤) طلب «أقوى/أفضل/فكرة أقوى» على صورة = أعطِ فكرة أبدع تبني على صورته لا نسخة شبه مطابقة؛ وطلب «نفس الصورة» = التزم بالحرفية. طلب «بدون أسماء/احذف الأسماء» = امسحها فعلًا. طلب «عدّل هذه النقطة» = عدّل ذلك الموضع فقط.\n' +
  '(ز٥) إن تعثّر توليد/تعديل الصورة (لم ترجع الأداة صورة): أعد استدعاء الأداة مرّة واحدة فورًا في نفس الردّ بلا استئذان. ممنوع منعًا باتًا أن تسأل «تبي أعيد المحاولة؟» — المستخدم طلب الصورة فأعِدها أنت. إن فشلت المرّتان قل بجملة واحدة قصيرة إن المولّد مشغول الآن وليُعِد المحاولة بعد لحظات، بلا سؤال.\n' +
...
const WIZARD_RE = /كتالوج|كتالوق|منيو|قائمة طعام|قائمة الطعام|بروفايل شرك/;
    ...
    /* v-maha-name (لقطة المالك ١٣ سبتمبر: «هلا مها، كيفك؟ وش تحتاجين»): «مها» اسم
       المساعدة الصوتيّة في التطبيق نفسه، والنموذج النصّيّ لم يكن يعرفه فظنّه اسم
       المستخدم وخاطبه به بالمؤنّث. */
    '«مها» هي المساعدة الصوتيّة في هذا التطبيق نفسه — أنت وهي مساعد واحد باسمين: إذا ناداك المستخدم «مها» فهو يخاطبك أنت، فردّ بصفتك مها (وبصيغة المؤنّث ما دام يخاطبك بها). «مها» ليست اسم المستخدم أبدًا ولا تنادِه بها.\n' +
    /* v-no-provider-names (لقطة المالك ١٣ سبتمبر: «صاحب مشروع كلاود قيد التطوير»): اسم
       نموذج تسرّب من السياق فصار «مشروعًا» للمستخدم. قاعدة المالك: لا اسم مزوّد في
       أيّ نصّ يراه المستخدم. */
    'أسماء النماذج ومزوّدي الذكاء الاصطناعي (أيّ شركة أو نموذج) لا تُذكر للمستخدم أبدًا ولو وردت في سياقك — تقول «المحرّك» فقط؛ ولا تنسب إلى المستخدم «مشروعًا» أو «عملًا» باسم نموذج أو مزوّد، فليست هذه مشاريعه.\n' +
    ...
    /* v-topic-memory (شكوى المالك ١٤ سبتمبر «أبدّل الموضوع وأرجع للي قبله فكأنّي ما سألته شي»): الصياغة السابقة
       «مرجعك الوحيد… التاريخ خلفيّة فقط» علّمت النموذج النسيان عند العودة إلى موضوع سابق. */
    'قاعدة الموضوع (أولوية قصوى): رسالة المستخدم الأخيرة تحدّد الموضوع الحاليّ. إن كانت موضوعًا جديدًا فأجب عنه وحده كاملًا ولا تكمل السابق ولا تخلطه به من تلقاء نفسك. وإن عادت إلى موضوع سابق في هذه المحادثة فأنت تذكره كاملًا بتفاصيله وتبني عليه مباشرةً — لا تقل إنّك لا تعرفه ولا تطلب إعادته ولا تخلطه بما جاء بينهما. تاريخ المحادثة كلّه ذاكرتك الحاضرة، لا قائمة مهام تُعاد.\n' +
...
module.exports = async (req, res) => {
  ...
  const toolTurn = !quietSocialTurn && !__analyzeDoc;
    ...
    // الرسم التلقائيّ. تبقى الصورة عند الطلب الصريح أو تعديل صورة مرفقة فقط.
    ...
    // الفكرة»): بعد كلّ صورة يرسمها النموذج بنفسه في محادثة عاديّة، تقرير قصير
    // يشرح فكرتها. يخصّ generate_image فقط — لا تعديل صورة مرفقة (edit_image
    // يبقى صورةً بلا شرح)، ولا صور داخل كتلة كود/صفحة HTML.
    const IMAGE_REPORT_NOTE =
      '\n\n[تفسير بعد الصورة — إلزاميّ]: كلّما رسمتَ صورةً بأداة generate_image في ردّ محادثة عاديّ (وليست صورةً داخل كتلة كود ```html أو صفحة تبنيها)، اكتب بعد رمز الصورة مباشرةً تقريرًا قصيرًا يشرح فكرتها بهذا الشكل حرفيًّا:\n📋 تفسير الفكرة\nثمّ ٣ إلى ٥ نقاط أو أسطر توضّح: ماذا رسمت، والعناصر الرئيسيّة في الصورة ومدلولها، والفكرة أو الرسالة وراءها. موجز ومباشر بلا حشو، بلغة المستخدم. قاعدة السرعة تبقى: اكتب تمهيد نصّك أوّلًا ثمّ الصورة ثمّ هذا التفسير في نهاية الردّ. (تعديل صورة أرفقها المستخدم بـedit_image يبقى صورةً بلا شرح كما هي قاعدته.)';
  ...
  try {
    ...
    if (__freeLane) {
      ...
      // مقصور على toolTurn=false أو أيّ رسالة طويلة، ويُلغى فور انتهاء البثّ.
      // الحارس الصلب (٥ دقائق) يبقى شبكة الأمان ضدّ التعليق.
      // v-chat-keepalive3 (بلاغ المالك «الرسالة الطويلة ما ترد» مجددًا): كان النبض
      // مقصورًا على toolTurn=false، لكن رسالةً طويلةً تحوي كلمة بحث/سعر/أخبار أو
      // مقطع إنجليزي/كود (foreignTurn) تُبقي toolTurn=true فتُحرَم من النبض ويسقط
      ...
      while (true) {
        ...
        // v-chat-heartbeat REVERTED (كان يُبقي حارس خمول العميل حيًّا للأبد حين
        // يعمل النموذج بأدوات بلا نصّ، فيدور الطلب ٥ دقائق بلا قطع ولا تشخيص).
        // بدونه: صمتُ الخادم الحقيقي يقطعه حارس العميل (٤٥ث) فيهبط لمسار احتياط
        // أو يُظهر تشخيصًا سريعًا بدل التعليق الطويل. أحداث الأدوات الحقيقية
        // (status/↳) تبقى بايتات تُصفّر الحارس أثناء العمل الفعلي.
      ...
      const settledTools = await Promise.all(toolBlocks.map(async (cb) => {
        ...
        try {
          ...
          else if (cb.name === 'generate_image') { input.userText = String(lastUserText || '').slice(0, 800).replace(/\s*\[[^\[\]]*\]\s*$/, '').slice(0, 600); /* v-quad-fix: القصّ قبل الـregex — كانت O(n²) على النصّ الطويل. مع صورة مرفقة قد يتحوّل الرسم إلى تعديل على برو (v-nano-pro-edit) فيحتاج مهلة التعديل */ result = await runInClient(send, 'generate_image', input, lastUserHasImage ? 150000 : 75000); }
          // v-edit-image-tool: التعديل عالي الدقة (gpt-image للنصّي) قد يأخذ دقيقتين.
          else if (cb.name === 'edit_image') { input.userText = String(lastUserText || '').slice(0, 800).replace(/\s*\[[^\[\]]*\]\s*$/, '').slice(0, 600); /* v-quad-fix: القصّ قبل الـregex */ result = await runInClient(send, 'edit_image', input, 150000); }

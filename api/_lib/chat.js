    ...
    // إزالة تكرار المواقع عبر استدعاءات البحث في نفس الدور
      ...
      const settledTools = await Promise.all(toolBlocks.map(async (cb) => {
        ...
        try {
          ...
          else if (cb.name === 'generate_image') { input.userText = String(lastUserText || '').slice(0, 600); result = await runInClient(send, 'generate_image', input, 75000); }
          // v-edit-image-tool: التعديل عالي الدقة (gpt-image للنصّي) قد يأخذ دقيقتين.
          else if (cb.name === 'edit_image') { input.userText = String(lastUserText || '').slice(0, 600); result = await runInClient(send, 'edit_image', input, 150000); }

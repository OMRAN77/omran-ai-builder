  ...
  try {
    ...
    if (editImageBase64 && extras.length) {
      /* 🧩 دمج عدة صور في تصميم واحد
         ...
         different angles as is most natural to the scene»). لم تُختبَر حيًّا بعد أيضًا — إن فشلت هذي المرّة
         فالسقف الفعليّ (لا الصياغة) هو الأرجح؛ راجع DECISIONS.md. */
      parts.push({ text: 'You are given ' + (extras.length + 1) + ' separate reference images, attached in this exact order. For any reference image that shows a real human face, that exact person\'s identity — face shape, features, skin tone, hair and expression — MUST be reproduced with full fidelity in the output, precisely as photographed in that image: never invented, never averaged or blended with another person\'s face, never swapped onto the wrong body. This holds even when the task changes their pose, camera angle, clothing detail, or places them together in a brand-new shared scene — only the scene, pose and angle change; who each person actually is never changes. The same fidelity rule applies to any logo or exact text in a reference image.\n\nTASK: "' + cleanPrompt + '"\n\nNow combine every reference image into ONE single, cohesive, photorealistic result following that task exactly:\n1. Every reference image\'s subject MUST appear in the final result - never drop one.\n2. Arrange them exactly as the task asks (e.g. the people from the separate reference photos placed together, naturally, in one new shared scene; or a logo/text laid out with other elements).\n3. Any Arabic text must remain correct and readable.\nOutput a single finished image only.' });

...
module.exports = async (req, res) => {
  ...
  try {
    ...
    /* v-person-swap: «غيّر أشكال الأشخاص…» تغيير هوية مقصود — يسبق تبديل الحرف الذي كانت تلتقطه كلمة «الاسم» */
    const isPersonSwap = !!editImageBase64 && !isSceneUpgrade && !isRestyle && !isReimagine && !isElevate && isPersonSwapRequest(intentText);
    /* v-broad-edit: «رتّب الصور / خلها فقط صور / بدون كتابة» طلب مركّب واسع — برو بلا حارس، كل الأوامر معًا */
    ...
    // v-img-tafsir (طلب المالك «تفسير بعد الصورة»): مُرسِل موحّد لكل مسارات
    // الصورة — يحسب caption (تفسير الفكرة/تقرير التعديل) ويضمّه دائمًا. كانت
    ...
    async function sendImg(b64, mime, engine, report, verdict) {
    ...
    async function deliver(first, altFn) {
      const r = await settleCandidates(Object.assign({ first: first, altFn: altFn, deadlineOk: function () { return (Date.now() - __t0) < 80000; },
      ...
      await sendImg(r.best.b64, r.best.mime, r.engine, r.report, r.best.verdict);
    }
    ...
    if (editImageBase64 && extras.length) {
      ...
      parts.push({ text: __rawCreative ? rawCreativePrompt(cleanPrompt, intentText)
        ...
        : (isPersonSwap ? buildPersonSwapPrompt(cleanPrompt, intentText)
    ...
    /* v-nano-edit (مقارنة المالك: «نانو الأصلي» يعيد التخيّل بجرأة، وتطبيقنا
       ...
       v-edit-pro-revert (المالك بعد نشر v-edit-pro: «كلّ الصور غشّ — لا صورة نفس الطلب»): جرّبنا برو للتعديل
    ...
    let lastRescueErr = '';
    async function openaiRescueImage(promptOverride) {
    ...
    async function gptCandidate(prompt) {
      const b64 = await openaiRescueImage(prompt);
    ...
    const __gptAlt = process.env.OPENAI_API_KEY ? function () { return gptCandidate((__faithfulLane && !extras.length) ? __retryPrompt : ''); } : null;

    /* v-img-mix: المحرّكان معًا بالتوازي على الطلب نفسه، ثمّ deliver يقيس كلًّا منهما ويحكم بينهما بالتنفيذ أوّلًا ويُرسل صورة
       واحدة («النتيجة ١»). فشلهما معًا = سلسلة الإنقاذ نفسها أدناه. */
    if (__engineMix) {
      const both = await Promise.all([proCandidate().catch(function () { return null; }), gptCandidate().catch(function () { return null; })]);
      const got = both.filter(Boolean);
      got.forEach(function (c) { c.engine = 'mix:' + c.engine; });
      if (got.length) { await deliver(got, null); return; }
    }
    ...
    if (__textRoute && !__engineMix) {
      ...
      if (denseB64) {
        await deliver({ b64: denseB64, mime: 'image/png', engine: 'openai' }, proCandidate); /* v-img-honest: لم يُنفَّذ = برو مرّة */
        return;
      }

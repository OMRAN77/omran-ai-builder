...
async function judgeBest(opts) {
  try {
    ...
    const r = await fetch(JUDGE + opts.apiKey, {
      ...
      body: JSON.stringify({ contents: [{ parts }], generationConfig: { temperature: 0, maxOutputTokens: 64, thinkingConfig: { thinkingBudget: 0 } } }), /* v-flash-budget + v-flash-nothink: التفكير الافتراضي يلتهم السقف كاملًا = جواب فارغ = «A» دائمًا */
    });

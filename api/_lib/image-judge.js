...
/* v-quality-first (قرار المالك النهائي: «انسى التكلفة — أريد النتيجة»): الوضع
   المزدوج مُفعّل افتراضيًا مرة أخرى — نانو + gpt-image معًا والحكم يختار الأقوى
   والأقرب للشخص في كل صورة. IMAGE_DUO=off يوقفه لو رغب لاحقًا. */
function duoEnabled() {
  ...
  try {
    ...
    const rubric = opts.creative
      ? 'You are a top art director judging two AI redesigns of the SOURCE image for this request: "' + String(opts.prompt || '').slice(0, 600) + '".\n' +
        'Pick the candidate that (1) is the most striking, rich and professionally finished — prefer the bolder, more complete design over the timid one; (2) stays faithful to the subject, purpose and any real person\'s identity and key wording of the source; (3) renders any text correctly (Arabic must be correct and readable); (4) has no artifacts, garbled details or instruction text written into the image.'
      : opts.source

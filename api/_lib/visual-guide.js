...
const MODEL = process.env.VISUAL_GUIDE_MODEL || process.env.SCREEN_GUIDE_MODEL || 'gemini-flash-latest';
...
function promptFor(mode, question, lang) {
  ...
  const base = ar ? 'أنت مرشد بصري لشخص كفيف أو ضعيف البصر. أجب بالعربية الواضحة بنص يصلح للنطق. لا تستخدم Markdown أو رموزًا أو مقدمات. كن صادقًا ولا تخمّن نصًا أو خطرًا لا تراه.' : 'You are a visual guide for a blind or low-vision person. Reply in clear spoken English. Do not use Markdown, symbols, or an introduction. Be honest and never invent text or hazards you cannot see.';
...
module.exports = async function visualGuide(req, res) {
  ...
  try {
    ...
    const upstream = await fetch('https://generativelanguage.googleapis.com/v1beta/models/' + encodeURIComponent(MODEL) + ':generateContent?key=' + key, {
      ...
      body: JSON.stringify({ contents: [{ role: 'user', parts: [{ inline_data: { mime_type: image.mime, data: image.b64 } }, { text: promptFor(mode, body.question, body.lang) }] }], generationConfig: { temperature: 0.2, maxOutputTokens: 420 } })
    });

...
/* «فكرة ثانية/مختلفة/أقوى» و«عطني/اقترح فكرة» و«زي الذكاء الاصطناعي» و«مو شغل فوتوشوب» = تصميم جديد للموضوع نفسه.
   (لقطة المالك ٦ سبتمبر «عطني فكرة أقوى من هذي — شغل فوتوشوب»: الترقية أبقت التركيب وأضافت خطًّا وإطارًا، بينما Gemini
   أعاد بناء التصميم حول الموضوع نفسه. «أقوى» وحدها تبقى ترقية للتركيب نفسه؛ أمّا «فكرة» فطلب تصميم جديد.) */
/* «عطني فكرة» / «اقترح لي» على صورة مرفقة = أَرِني الأقوى (كتطبيق Gemini: «عطني فكرة أقوّم هذي الصورة» أخرج تصميمًا أغنى) —
   لا «عطني فكرة لاسم/نص/شعار» (طلب كلام لا صورة). «فكرة ثانية/مختلفة» تبقى مشهدًا جديدًا (reimagine يُفحص قبلها). */
const AR_IDEA = '(?:^|[\\s،,])(?:عطني|أعطني|اعطني|هات|هاتي|اقترح|أقترح|اقترحي|ابي|أبي|ابغى|أبغى|ابغي|أبغي|أريد|اريد|ودي|ودّي|ممكن)\\s*(?:لي\\s*)?(?:فكر[ةه]|أفكار|افكار|اقتراح|اقتراحات)(?!\\s*(?:ل|عن|بخصوص|اسم|نص|كلام|عبار[ةه]|شعار|تعليق|كابشن|وصف|هاشتاق|نكت[ةه]))';
/* «خلها زي الذكاء الاصطناعي» / «سوها بالذكاء الاصطناعي» / «شغل ذكاء اصطناعي مو فوتوشوب» = الأقوى لا لمسة فوتوشوب */
const AR_AI_STYLE = '(?:زي|مثل|كأنه|كانه|كإنه|كأنها|كانها|شغل|بال|ب)\\s*(?:ال)?ذكاء\\s*(?:ال)?(?:اصطناعي|إصطناعي|صناعي)|\\b(?:ai|a\\.i\\.)[-\\s]?(?:style|generated|look|art|quality|magic)\\b|\\blook\\s+ai[-\\s]?generated\\b';
const AR_NOT_PHOTOSHOP = '(?:مو|مش|ليس|لا|بدون|بلا|غير)\\s*(?:شغل\\s*)?فوتو\\s*شوب|شغل\\s*فوتو\\s*شوب|\\b(?:not|no)\\s+photoshop\\b';
const REIMAGINE_RE = new RegExp([
  'فكر[ةه]\\s*(?:ثانية|ثانيه|مختلفة|مختلفه|جديدة|جديده|غير|أقوى|اقوى|أفضل|افضل|أحسن|احسن|أحلى|احلى|أجمل|اجمل|أفخم|افخم|أبدع|ابدع|أروع|اروع)',
  /* لقطة المالك ٦ سبتمبر «عطني صورة نفس الفكره لكن أجمل منها — غيّر شي بسيط»: «نفس الفكرة» + أقوى/أجمل/تصميم = الفكرة نفسها بتصميم جديد */
  'نفس\\s*الفكر[ةه](?:\\s*(?:بس|لكن|لاكن|مع|و|ب|في))?\\s*(?:ال)?(?:أقوى|اقوى|أجمل|اجمل|أفخم|افخم|أحلى|احلى|أفضل|افضل|أرقى|ارقى|أروع|اروع|أبدع|ابدع|تصميم|ستايل|شكل|طريق[ةه])',
  'غيّ?ر\\s*الفكرة', 'بشكل\\s*مختلف\\s*تمام', 'مختلف\\s*تمام', 'تصميم\\s*ثاني', 'ستايل\\s*ثاني', 'بدّ?ل\\s*(?:الفكرة|التصميم|الستايل)',
  AR_IDEA + END, AR_AI_STYLE, AR_NOT_PHOTOSHOP,
  'different\\s*(?:idea|concept|style)', 'new\\s*concept', 'another\\s*(?:idea|take|concept)', 'reimagine',
  '\\b(?:give\\s+me|suggest|got|any)\\s+(?:an?\\s+|some\\s+)?(?:ideas?|suggestions?)\\b(?!\\s+for\\s+(?:a\\s+|the\\s+)?(?:name|caption|slogan|title|text|tagline))',
].join('|'), 'i');
...
const AR_ELEVATE_VERBS = '(?:[اأ]?(?:طوّ?ر|حسّ?ن|جمّ?ل|قوّ?م?|رقّ?|زيّ?ن|عزّ?ز|زخرف|فخّ?م)|ارفع|إرفع|ارتقِ?|ابهر|أبهر)';
/* «سوّي لي شي يجنن» / «عطني نسخة رهيبة» / «خلها تخبل» — الصفة العامية تُعدّ ترقية فقط مع «شي/نسخة/خلها» (بارِدةً وحدها هي مجاملة) */
const AR_SLANG_ADJ = '(?:يجنن|تجنن|يخبل|تخبل|يهبل|تهبل|خرافي[ةه]?|أسطوري[ةه]?|اسطوري[ةه]?|رهيب[ةه]?|خيال|فخام[ةه])';
const AR_SLANG_LEAD = '(?:(?:سوّ?ي?|سو|اعمل|أعمل|عطني|أعطني|اعطني|هات|ابي|أبي|ابغى|أبغى|ابغي|أبغي|أريد|اريد|ودي|ودّي|طلع|طلّع)\\s*(?:لي\\s*)?(?:شي|شيء|نسخ[ةه]|تصميم|صور[ةه]|شكل)\\s*(?:\\S+\\s+)?|(?:خلّ?ي?ها|سوّ?ي?ها|اجعلها|صيّ?رها|خلّ?ي?ه|سوّ?ي?ه|اجعله|صيّ?ره)\\s*)';
...
const ELEVATE_RE = new RegExp([
  ...
  '(?:^|[\\s،,])' + AR_ELEVATE_VERBS + '(?:(?:ها|ه|يها|يه|ي|ني)' + END + '|\\s*(?:(?:هذي|هذه|هذا|ذي)\\s*|ه)?' + AR_WHOLE_OBJ + END + ')',
  '(?:^|[\\s،,])' + AR_SLANG_LEAD + AR_SLANG_ADJ + END,

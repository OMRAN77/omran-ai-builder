...
const RESTYLE_RE = /(^|[\s،,])(3d|ثلاثي|مجسم|مجسّم|كرتون|كارتون|أنيمي|انمي|بيكسار|ديزني|بكسل|بيكسل|سايبر|نيون|كوميك|كومكس|مانجا|فانتازيا|واقعي|anime|cartoon|pixar|disney|pixel\s*art|cyberpunk|neon\s*(?:style|look|art)|comic|manga|fantasy|watercolor|oil\s*paint(?:ing)?|sketch|realistic|3d\s*render)(?:ي[ةه]?|[ةه]|ات)?(?=$|[\s،,.!؟?])|(?:^|[\s،,])رسم[ةه]?\s*(?:ب)?(?:ال)?رصاص(?=$|[\s،,.!؟?])|(?:^|[\s،,])(?:لوح[ةه]|رسم[ةه]?|ألوان|الوان|ستايل|أسلوب|اسلوب|طابع)\s*(?:ال)?(?:زيتي[ةه]?|مائي[ةه]?)(?=$|[\s،,.!؟?])|\b(?:redesign|restyle|re-?render|(?:architectural|photorealistic|cinematic)\s+render(?:ing)?|render\s+(?:it|this|the\s+(?:whole|entire)\s+\w+)|in\s+(?:a|an|the)\s+(?!same\b)[^.\n]{0,60}?\bstyle)\b/i;
...
const AR_NOT_PREFER = '(?!\\s+(?:ال\\S|(?:لو|أن|ان|إن|إنك|انك|إنه|انه)(?=[\\s،,]|$)|تخل|تسو|تغي|تحط|تكتب|تشيل|تضيف|تحذف))';
...
const ELEVATE_RE = new RegExp([
  ...
  '(?:^|[\\s،,])(?:فخم[ةه]?|فخام[ةه]|راقي[ةه]?|رايق[ةه]?|خيالي[ةه]?|جبار[ةه]?|مبهر[ةه]?|احترافي[ةه]?\\s*(?:أكثر|اكثر)?|إبداعي[ةه]?|ابداعي[ةه]?|تجنن|لايق[ةه]?)(?=$|[\\s،,.!؟?])',
  ...
  '(?:^|[\\s،,])(?:زوّ?د|زيد|كثّ?ر|ضاعف)\\s*(?:ال)?(?:زخارف|زخرف[ةه]|تفاصيل|إبداع|ابداع|جمال|فخام[ةه]|إبهار|ابهار|روع[ةه])(?=$|[\\s،,.!؟?])',
...
/* عنصر محدّد داخل الصورة: «حط إطار أفخم» و«اجعل الخلفية أجمل» و«اكتب اسم عمران بخط احترافي» تعديلات
   موضعية — الصفة تصف العنصر لا الصورة. الترقية الكاملة فقط حين تصف الصفة الصورة كلها أو تتصدّر الطلب. */
const LOCAL_OBJECT_RE = /(?:^|[\s،,])(?:ال|لل|بال|وال|و|ب)?(?:إطار|اطار|برواز|خلفي[ةه]|خط|نص|كلم[ةه]|كلمات|جمل[ةه]|عبار[ةه]|اسم|أسماء|اسماء|رقم|أرقام|ارقام|تاريخ|لون|ألوان|الوان|شعار|لوجو|عنوان|إضاء[ةه]|اضاء[ةه]|نور|ظل|ظلال|توهج|تأثير|فلتر|زر|أيقون[ةه]|ايقون[ةه]|حدود|حاف[ةه]|زاوي[ةه]|جزء|منطق[ةه]|جهة|يمين|يسار|أعلى|اعلى|أسفل|اسفل|فوق|تحت|سماء|سما|غيوم|قب[ةه]|مئذن[ةه]|شجر|أشجار|اشجار|نخل|نجوم|قمر|شمس|وجه|شعر|عيون|عين|فم|ابتسام[ةه]|ملابس|قميص|فستان|ثوب|شماغ|نظار[ةه]|سيار[ةه]|طاول[ةه]|كرسي|جدار|أرضي[ةه]|ارضي[ةه]|سقف|باب|نافذ[ةه]|شباك|لمس[ةه]|لمسات|frame|border|background|font|text|word|name|number|date|title|caption|color|colour|logo|lighting|light|shadow|glow|effect|filter|button|icon|corner|edge|part|area|sky|dome|face|hair|eyes|shirt|dress|car|table|chair|wall|floor|ceiling|door|window)(?:ي|نا|ك|كم|ها|ه|ات|ين)?(?=$|[\s،,.!؟?])/i;
const LEAD_ELEVATE_RE = new RegExp('^\\s*(?:(?:ابي|أبي|ابغى|أبغى|ابغي|أبغي|أريد|اريد|ودي|ودّي|عطني|أعطني|اعطني|هات|سو|سوي|سوّي|سولي|سوّلي|خل|خلي|خلها|خليها|خلّها|خلّيها|اجعلها|صيرها|صيّرها|طلع|طلّع|طلعلي)\\s*(?:لي\\s*)?)?(?:(?:نسخ[ةه]|صور[ةه]|شكل|تصميم|كرت|بطاق[ةه]|شي|شيء|الكرت|الصور[ةه]|التصميم|البطاق[ةه])\\s*)?(?:ال)?' + AR_ELATIVE + '(?=$|[\\s،,.!؟?])', 'i');
...
function detectEditIntent(text) {
  ...
  const elevate = !restyle && !reimagine && !sameImage && ELEVATE_RE.test(s) && (!LOCAL_OBJECT_RE.test(s) || LEAD_ELEVATE_RE.test(s));
  const placeUpgradeHint = PLACE_RE.test(s) || APPLY_RECS_RE.test(s);
  return { sameImage, restyle, reimagine, elevate, placeUpgradeHint };
}

module.exports = { detectEditIntent, SAME_IMAGE_RE, RESTYLE_RE, REIMAGINE_RE, ELEVATE_RE, LOCAL_OBJECT_RE, LEAD_ELEVATE_RE, PLACE_RE, APPLY_RECS_RE };

# إضافة «عمران AI» للمتصفّحات — Chrome وEdge وFirefox
...
| تحديد نصّ في أيّ صفحة ← زرّ أيمن ← «اسأل عمران AI عن …» | يفتح التطبيق والنصّ المحدّد في الصندوق |
...
## البناء
```
npm run extension:zip     # ← store/browser-extension/dist/omran-ai-1.0.0.zip
```
...
### نصوص المتجر
- **الاسم:** عمران AI / Omran AI (يأتي من `_locales` تلقائيًّا)
- **الفئة:** Productivity / الإنتاجيّة
- **الوصف القصير:** من `extDesc` في `_locales/<اللغة>/messages.json`
- **الوصف الطويل (عربيّ):** عمران AI مساعدك الذكيّ في كلّ صفحة. حدّد أيّ نصّ واضغط بالزرّ الأيمن لتسأل عنه، أو اكتب
  `om` في شريط العنوان، أو اضغط زرّ الإضافة واكتب سؤالك. يفتح عمران AI ومعه سؤالك جاهزًا: محادثة، إنشاء تطبيقات
  ومواقع، صور وفيديو، مستندات، تعليم، قبلة ومواقيت، وأكثر.
- **Long description (English):** Omran AI on every page. Select any text and right-click to ask about it, type `om`
  in the address bar, or click the toolbar button and type your question. Omran AI opens with your question ready:
...
- **الغرض الوحيد (Single purpose):** فتح عمران AI بسؤال المستخدم أو النصّ الذي حدّده.
- **لماذا `contextMenus`:** لإضافة «اسأل عمران AI» إلى قائمة الزرّ الأيمن على النصّ المحدّد.

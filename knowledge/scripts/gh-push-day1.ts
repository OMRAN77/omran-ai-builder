// يدفع ملفات يوم ١ إلى GitHub main ويتحقّق بالبصمة
...
const files = ['api/_lib/agent.js', 'api/_lib/agent-tool-result.js', 'api/_lib/memory.js'];
...
const res = await invokeTool({
  ...
  args: { owner, repo, branch, commitMessage: 'day1: lock agent-tool-result endpoint, wire user memory into agent, honest no-tools fallback notice, 12s->25s client wait', files: files.map(f => ({ repoPath: f, localPath: `${SRC}/${f}` })) },
});

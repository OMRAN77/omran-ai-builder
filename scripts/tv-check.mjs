/* فاحص قنوات التلفزيون — يشتغل على GitHub Actions يوميًّا.
 * v-tv-no-youtube (المالك ٢٣ سبتمبر: «مااريد شي اسمه يوتيوب — اريد شغل مباشر»): التطبيق يشغّل البثّ المباشر
 * داخله فقط (HLS)، فحُذف قسم يوتيوب كاملًا (حلّ المعرّفات والبحث وكشف «الحيّ» — ميّت منذ ٤ سبتمبر ويأكل
 * ~١٠ دقائق). الباقي: فحص روابط البثّ عميقًا، قائمة الرياضة الطازجة، وجدول المباريات ← tv-status.json.
 */
...
/* v-tv-matches: جدول المباريات من لوحة ESPN العامّة — الجولة الحاليّة لكلّ دوري (بلا تاريخ)
...
const out = {
  ...
  counts: { streamsOk: mOk, streamsCors: mCors, streamsDeep: deepCounts, sportsFresh: sportsOut.length, matches: matches.length },
  streams: streamsStatus,
  sports: sportsOut,
  matches,
};
await writeFile('tv-status.json', JSON.stringify(out) + '\n');
console.log('\nالخلاصة: ' + mOk + ' رابط شغّال، ' + sportsOut.length + ' قناة رياضيّة، ' + matches.length + ' مباراة في الجدول.');

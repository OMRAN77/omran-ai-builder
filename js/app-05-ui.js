...
(function(){
  ...
  b.onclick = async (e) => {
    e.stopPropagation();
    if(!(await window.omranConfirmDelete({ message: t('confirmDeleteChat'), itemLabel: (state.projects.find(p => p.id === state.currentId) || {}).title || t('deleteChatLabel') }))) return;
